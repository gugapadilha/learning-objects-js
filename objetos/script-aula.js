...
const frutose = ['', 'Melancia', 'Abacate', 'Melão'];

const every2 = frutose.every((fruta) =>{
  console.log(fruta);
  return fruta;
});

const findIndex = frutose.findIndex(item =>{ 
  return item;
});
...
const frutasF = ['Banana', undefined, null, '', 'Uva', 0, 'Maçã'];

const arrayFrutas = frutasF.filter((item) =>{
  return item 
});
